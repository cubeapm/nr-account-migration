...
def migrate(
    ...
    all_queries = []
    for widget in dashboard_widgets.get('widgets', []):
        if 'rawConfiguration' in widget and 'nrqlQueries' in widget['rawConfiguration']:
            for query in widget['rawConfiguration']['nrqlQueries']:
                if 'query' in query:
                    all_queries.append(query['query'])

    logger.info('Processing %d NRQL queries from dashboard widgets' % len(all_queries))

    for query in all_queries:
        res = re.search(guidRegEx, query, flags=re.IGNORECASE)
        if not res:
            continue
        ...
        for guid in guids:
            ...
            if idType == 'appId':
                ...
                if res['entityFound'] and res['entity']:
                    ...
                    logger.info('Added missing entity: %s (GUID: %s)' % (res['entity'].get('name', 'Unknown'), guid))
                else:
...
def save_entities_extended(account_id, entities):
    ...
    logger.info('Saved extended entities: %d total entities' % len(entities))
...
def create_argument_parser():
    parser = argparse.ArgumentParser(
        description='Extend entity data for dashboard widgets'
    )
